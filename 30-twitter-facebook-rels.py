...
def _map(arg):
  ...
  for name in names:
    ...
    twitters = [twitter.text for twitter in soup.find_all('li', {'class':'twitter'}) if 'フォロワー' in twitter.text]
    if len(twitters) != 1:
      continue

    try:
      followers = re.search(r'(\d{1,})', twitters.pop().strip()).group(1)
    except:
      continue
    try:
    ...
    result = f'{followers}, {target}, {number}'
